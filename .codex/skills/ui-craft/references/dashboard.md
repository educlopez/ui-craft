...
- **Every table is wrapped in `overflow-x: auto`, and any table over ~15 rows gets `position: sticky; top: 0` on `thead`.** **Why:** a table is the one layout that cannot reflow — columns have minimum content widths, so at ~320px it either overflows the viewport and clips or forces the whole page to scroll sideways. And once the header scrolls out of view, every cell below it is an unlabelled string; the user re-scrolls to the top to remember which column is which. Both are one line of CSS and neither is optional on an operator surface, where the table *is* the product.
