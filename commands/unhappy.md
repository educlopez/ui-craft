...
**Convergence note:** To iterate until all required states are present, load `../skills/ui-craft/references/loops.md` and run preset `state-coverage` (budget 3): after stubbing the highest-priority missing required state, re-inventory until all knob-required states are present or budget exhausted. Emit the pre-flight cost notice before iteration 1.
